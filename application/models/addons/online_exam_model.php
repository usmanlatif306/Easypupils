...
class online_exam_model extends CI_Model
{
	...
	public function store()
	{
		...
		if ($_FILES['attachment']['name']) {
			if (!file_exists('uploads/live_class')) {
				mkdir('uploads/live_class', 0777, true);
			}
			$file_ext = pathinfo($_FILES['attachment']['name'], PATHINFO_EXTENSION);
			$data['attachment'] = md5(rand(10000000, 20000000)) . '.' . $file_ext;
			move_uploaded_file($_FILES['attachment']['tmp_name'], 'uploads/live_class/' . $data['attachment']);
		}
		...
		$response = array(
			'status' => true,
			'notification' => get_phrase('online_exam_added_successfully')
		);
	...
	public function update()
	{
		...
		if ($previous_data) {
			...
			$data['session'] = $this->active_session;
			if ($_FILES['attachment']['name']) {
				if (!file_exists('uploads/online_exam')) {
					mkdir('uploads/online_exam', 0777, true);
				}
				$file_ext = pathinfo($_FILES['attachment']['name'], PATHINFO_EXTENSION);
				$data['attachment'] = md5(rand(10000000, 20000000)) . '.' . $file_ext;
				move_uploaded_file($_FILES['attachment']['tmp_name'], 'uploads/online_exam/' . $data['attachment']);
			}
			...
			$response = array(
				'status' => true,
				'notification' => get_phrase('online_exam_updated_successfully')
			);
		} else {
			$response = array(
				'status' => false,
	...
	public function get_live_class_settings($user_id = "")
	{
		...
		$settings_data = $this->db->get_where('online_exam_settings', ['school_id' => $this->school_id, 'user_id' => $user_id])->row_array();
	...
	public function get_live_class_by_id($id)
	{
		...
		$data = $this->db->get_where('online_exam', $checker);
		return $data;
	}
	...
	public function update_settings()
	{
		...
		$previous_data = $this->db->get_where('online_exam_settings', ['school_id' => $this->school_id, 'user_id' => $user_id]);
		if ($previous_data->num_rows()) {
			$this->db->where('user_id', $user_id);
			$this->db->where('school_id', $this->school_id);
			$this->db->update('online_exam_settings', $data);
		} else {
			$this->db->insert('online_exam_settings', $data);
		}

		$response = array(
			'status' => true,
			'notification' => get_phrase('online_exam_settings_updated_successfully')
		);
		return json_encode($response);
	}
