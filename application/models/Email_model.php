...
class Email_model extends CI_Model
{
	...
	// Sending test
	function parent_email($to, $message)
	{
		$email_sub = "Message From " . get_settings('system_name');
		$email_message  = '<html><body><h4>Dear Parent,</h4><p>Greetings,</p><p>' . $message . '</p><p>Thanks,</p><h4>' . get_settings('system_name') . '</h4></body></html>';
		$email_to = $to;

		$this->send_mail_using_smtp($email_message, $email_sub, $email_to);

		if (get_smtp('mail_sender') == 'php_mailer') {
			$this->send_mail_using_php_mailer($email_message, $email_sub, $email_to);
		} else {
			$this->send_mail_using_smtp($email_message, $email_sub, $email_to);
		}
		return true;
