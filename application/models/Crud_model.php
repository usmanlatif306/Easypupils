...
class Crud_model extends CI_Model
{
	...
	public function take_attendance()
	{
		...
		if ($check_data->num_rows() > 0) {
			foreach ($students as $key => $student) :
				...
				$student = $this->db->get_where('students', array('id' => $student))->row_array();
				...
				$this->email_model->parent_email($to, 'Teacher has marked attendance in school');
			endforeach;
		} else {
	...
	public function event_calendar_create()
	{
		...
		$parents = $this->db->get_where('parents', array('school_id' => $data['school_id']))->row_array();
		foreach ($parents as $parent) {
			$to = $this->user_model->get_user_details($parent['user_id'], 'email');
			$this->email_model->parent_email($to, 'Teacher has added new calender event in school.');
		}
	...
	public function mark_update()
	{
		...
		if ($query->num_rows() > 0) {
			...
			// sending mail to parent about mark updated
			$student = $this->db->get_where('students', array('id' => $data['student_id']))->row_array();
			$parent = $this->db->get_where('parents', array('id' => $student['parent_id']))->row_array();
			$to = $this->user_model->get_user_details($parent['user_id'], 'email');
			$this->email_model->parent_email($to, 'Teacher has updated marks in school');
		} else {
