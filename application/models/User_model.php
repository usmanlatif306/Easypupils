...
class User_model extends CI_Model
{
	...
	{
		$data['user_id'] = html_escape($this->session->userdata('user_id'));
		$data['receiver_id'] = html_escape($this->input->post('receiver_id'));
		...
		$response = array(
			'status' => true,
			'notification' => get_phrase('message_send_successfully'),
		);
		return json_encode($response);
	}
	...
	public function delete_message($id)
	{
		...
		$response = array(
			'status' => true,
			'notification' => get_phrase('message_deleted_successfully'),
		);
		return json_encode($response);
	}

	// Saving message
	public function save_my_file()
	{
		// date_added
		$data['user_id'] = html_escape($this->session->userdata('user_id'));
		$data['display_name'] = $_FILES['file']['name'];
		$file_extension = pathinfo($_FILES['file']['name'], PATHINFO_EXTENSION);
		$data['file'] = md5(rand(10000000, 20000000)) . '.' . $file_extension;
		move_uploaded_file($_FILES['file']['tmp_name'], 'uploads/my_files/' . $data['file']);
		date_default_timezone_set("Asia/Karachi");
		$data['created_at'] = date('Y-m-d H:i:s');
		$this->db->insert('my_files', $data);
