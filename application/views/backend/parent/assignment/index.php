...
// $parent = $this->db->get_where('parents', array('user_id' => $user_id))->row_array();
// $student = $this->db->get_where('students', array('parent_id' => $parent['id']))->row_array();
// $enrolment = $this->db->get_where('enrols', array('student_id' => $student['id']))->row_array();
// $this->db->where('class_id', $enrolment['class_id']);
...
<script>
    ...
    function filterAssignment() {
        ...
        $.ajax({
            type: 'post',
            url: '<?php echo site_url('addons/assignment/filter_student_assignment_for_parent/' . $assignment_type); ?>',
            data: {
                class_id: class_id,
