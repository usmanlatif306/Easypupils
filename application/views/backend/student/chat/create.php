...
$school_id = school_id();
$users = $this->db->get_where('teachers', array('school_id' => $school_id))->result_array();
        ...
        <div class="form-group col-md-12">
            ...
            <select class="form-control select2" data-toggle="select2" id="receiver_id" name="receiver_id" required>
                ...
                <?php foreach ($users as $user) : ?>
                    <option value="<?php echo $this->db->get_where('users', array('id' => $user['user_id']))->row('id'); ?>"><?php echo $this->db->get_where('users', array('id' => $user['user_id']))->row('name'); ?></option>
                <?php endforeach; ?>
            </select>
        </div>
