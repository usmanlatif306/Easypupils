    ...
    <div class="col-md-4 toll-free-box text-center text-white pb-2" style="background-color: #6c757d; border-radius: 10px;">
        ...
        <span><?php echo get_phrase('subject'); ?> :
            <?php if ($subject_id === 'all') {
                ...
                echo $this->db->get_where('subjects', array('id' => $subject_id))->row('name');
            }
            ?>
        </span>
    </div>
    ...
    <table class="table table-bordered table-responsive-sm" width="100%">
        ...
        <tbody>
            <?php foreach ($marks as $mark) : ?>
                <?php if ($mark['student_id'] == $student_data['id']) : $subject = $this->db->get_where('subjects', array('id' => $mark['subject_id']))->row_array(); ?>
                    <tr>
